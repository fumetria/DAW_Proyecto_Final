            ...
            <div className="max-h-[90vh] w-full max-w-lg overflow-hidden rounded-xl bg-white shadow-xl dark:bg-slate-800 dark:text-slate-100">
                ...
                <div className="max-h-[calc(90vh-8rem)] overflow-y-auto p-4">
                    {loading ? (
                        ...
                        <p className="text-center text-stone-500">No se encontró el ticket.</p>
                    ) : (
                        <div className="space-y-4">
                            <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-sm">
                                <dt className="font-medium text-stone-500 dark:text-slate-400">Fecha:</dt>
                                <dd>{formatDate(detail.created_at)}</dd>
                                <dt className="font-medium text-stone-500 dark:text-slate-400">Usuario:</dt>
                                <dd>{detail.user_email}</dd>
                                <dt className="font-medium text-stone-500 dark:text-slate-400">Total:</dt>
                                <dd className="font-semibold">{formatPrice(detail.total)}</dd>
                            </dl>
                            <div>
                                <h3 className="mb-2 text-sm font-semibold text-stone-600 dark:text-slate-300">Líneas</h3>
                                <table className="w-full text-left text-sm">
                                    <thead>
                                        <tr className="border-b border-stone-200 dark:border-slate-600">
                                            ...
                                            <th className="py-1.5 text-right font-medium w-14">Cant.</th>
                                        ...
                                        {detail.lines.map((line) => (
                                            <Fragment key={line.id}>
                                                <tr className="border-b border-stone-100 dark:border-slate-700">
                                                    <td className="py-1.5 pr-2 align-top">{line.cod_art}</td>
                                                    <td className="py-1.5 pr-2 align-top">{line.article_name ?? "—"}</td>
                                                    <td className="py-1.5 text-right align-top">{line.quantity}</td>
                                                    <td className="py-1.5 text-right align-top">{formatPrice(line.price)}</td>
                                                    <td className="py-1.5 text-right align-top">{formatPrice(line.total)}</td>
                                                </tr>
                                                {line.details?.trim() ? (
                                                    <tr className="border-b border-stone-100 dark:border-slate-700">
                                                        <td className="pb-1.5 pt-0 pr-2" />
                                                        <td colSpan={4} className="pb-1.5 pt-0 pr-2 text-stone-500 dark:text-slate-400 text-xs italic">
                                                            {line.details.trim()}
                                                        </td>
                                                    </tr>
                                                ) : null}
