...
import { CategoryFormState, updateCategory } from "@/app/lib/actions";
...
export default function EditCategoryModal({
  ...
  const [state, formAction] = useActionState<
    ...
    FormData
  >(
    (prev: CategoryFormState | null, fd: FormData) =>
      updateCategory(category ? category.id : 0, prev, fd),
    null,
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl dark:bg-slate-800 dark:text-slate-50">
        <div className="flex justify-between items-center bg-stone-700 dark:bg-slate-700 text-stone-100 dark:text-slate-50 px-4 py-3 rounded-t-xl">
          <h3 className="font-semibold text-lg">Editar categoría</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form action={formAction} aria-describedby="form-error">
          <div className="rounded-md bg-stone-100 dark:bg-slate-800 p-4 md:p-6 dark:text-slate-50">
            <div className="grid mb-4">
              <label htmlFor="category-name" className="mb-2 text-sm">
                Nombre Categoría
              </label>
              <input
                type="text"
