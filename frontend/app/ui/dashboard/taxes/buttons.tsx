...
import { deleteTax } from "@/app/lib/actions";

interface ModalButtonProps {
  onOpen: () => void;
}
export function CreateTax({ onOpen }: ModalButtonProps) {
  return (
    <button
      type="button"
    ...
    >
      <p className="hidden md:block">Nuevo tipo IVA</p>
      <FontAwesomeIcon icon={faPlus} />
    </button>
  );
}

export function DeleteTax({ id }: { id: number }) {
  return (
    <ConfirmModal
...
export function UpdateTax({ onOpen }: ModalButtonProps) {
  return (
    <button
      type="button"
      title="Editar tipo de IVA"
      onClick={onOpen}
