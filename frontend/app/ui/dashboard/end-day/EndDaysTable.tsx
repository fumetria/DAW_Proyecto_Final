...
import type { EndDayRow } from "@/app/lib/end-day.action";
        ...
        <section>
            <table className="bg-stone-100 dark:text-slate-50 dark:bg-slate-800 min-w-full rounded-xl">
                ...
                <tbody>
                    {endDays?.length === 0 ? (
                        <tr>
                            <td colSpan={5} className="p-6 text-center text-stone-500 dark:text-slate-400">
                        ...
                        endDays?.map((row) => (
                            ...
                            >
                                <td className="px-2 py-3 text-center">{formatDateStr(row.date)}</td>
