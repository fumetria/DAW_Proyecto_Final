...
CREATE TABLE "payment-methods" (
	"id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "payment-methods_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"name" varchar(255) NOT NULL,
	"updated_at" timestamp,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"deleted_at" timestamp,
	CONSTRAINT "payment-methods_name_unique" UNIQUE("name")
);
--> statement-breakpoint
...
ALTER TABLE "articles" ADD CONSTRAINT "articles_tax_id_taxes_id_fk" FOREIGN KEY ("tax_id") REFERENCES "public"."taxes"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "end-days" ADD CONSTRAINT "end-days_user_email_users_email_fk" FOREIGN KEY ("user_email") REFERENCES "public"."users"("email") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "receipts" ADD CONSTRAINT "receipts_payment_method_payment-methods_id_fk" FOREIGN KEY ("payment_method") REFERENCES "public"."payment-methods"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE VIEW "public"."receipt_view" AS (select "receipts"."id", "receipts"."num_receipt", "receipts"."created_at", "receipts"."total", "payment-methods"."name" as "payment_method_name", "receipts"."user_email", "receipts"."is_open" from "receipts" left join "payment-methods" on "receipts"."payment_method" = "payment-methods"."id");